...
fun FilterContent(
    ...
    val animatedAlpha by animateFloatAsState(
        targetValue = if (isClosing) 0f else 1f, animationSpec = tween(300), label = "content_alpha"
    )
    ...
    Column(
        ...
        // Header Section
        FilterHeader(Modifier.padding(horizontal = 24.dp))
        ...
        FilterSection(
            ...
            CategoryFilterChips(
                selectedCategory = selectedCategory, onCategorySelected = onCategorySelected
            )
        }
        ...
        FilterSection(
            ...
            SortFilterChips(
                selectedSortBy = selectedSortBy, onSortBySelected = onSortBySelected
            )
        }
        ...
        FilterSection(
            Modifier.padding(horizontal = 24.dp),
            title = "Region", subtitle = "Filter by specific countries", icon = Icons.Filled.Public
        ) {
            CountryFilterChips(
                selectedCountry = selectedCountry, onCountrySelected = onCountrySelected
            )
        }
